...
def main() -> None:
    ...
    require("老師/志工協助" in support, "Student support must use one shared staff lane.")
    require(
        "老師與志工都會收到同一筆接力" in teacher,
        "Teacher queue must explain shared handoff.",
    )
    require(
        "只顯示目前服務班級中，學生主動送出的求助" in volunteer,
        "Volunteer queue must explain its least-privilege shared handoff.",
    )
    require(
        "任何一端回覆都會消除雙方紅點" in teacher
        and "任一方回覆後，雙方提醒會同步更新" in volunteer,
    ...
    require(
        "markSupportThreadHandledWithoutReply" not in teacher + volunteer,
        "Removed read-without-reply action must not return to staff UI.",
    )
    require(
        "StaffOriginalStudentMessageCard" not in teacher,
        "Teacher side must not duplicate the student message card.",
    )
    require(
        "VolunteerOriginalStudentMessageCard" not in volunteer,
        "Volunteer side must not duplicate the student message card.",
    )
    require(
        "StaffMissingQuestionSnapshotLabel" in teacher,
        "Teacher side must keep a low-priority missing-snapshot label.",
    )

    require(
        "teacherArchivedAt" in models and "volunteerArchivedAt" in models,
        "Support archive state must remain role-specific.",
    )
    require(
        "countsTowardSharedStaffBadge" in models,
        "Shared badge logic must remain role-aware.",
    )
    require(
        "hasAnyStaffAction" in models,
