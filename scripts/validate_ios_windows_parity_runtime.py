...
def main() -> int:
    ...
    require("TextField(\"email\"" in text["login"], "Login screen must show an email field", errors)
    require("SecureField(\"password\"" in text["login"], "Login screen must show a password field", errors)
    require("測試帳號登入" in text["login"], "Login copy must state test account login", errors)
    require("角色與班級資料" in text["login"], "Login copy must state role and class membership checks", errors)
    require("退回本機展示帳號" not in text["login"], "Login copy must not promise local demo fallback", errors)
    ...
    require("roleMismatch(expected: expectedRole, actual: role)" in text["firebase_auth"], "FirebaseAuthService must reject role mismatch", errors)
