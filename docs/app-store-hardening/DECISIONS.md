...
| D-27 | 2026-07-16 | App Store development uses an isolated production Firebase project, Worker, R2 bucket, administrator build and Xcode Release configuration; the submitted MAIC build 52 remains on the competition environment. | A production release must not mutate the backend or TestFlight group used by competition judges. |
| D-28 | 2026-07-16 | Public self-registration is available only to users who confirm they are at least 13. Users under 13 use a distinct school- or guardian-managed onboarding path; English+ will not use Apple's Kids Category for this release. | Keeps the public account path explicit while avoiding collection of an exact birth date and preventing under-13 users from independently completing the adult-style consent flow. |
| D-29 | 2026-07-16 | Before a teacher account is deleted, each owned class is transferred to an eligible active co-teacher selected or confirmed by the owner. If no eligible co-teacher exists, the class is archived and its history is retained under the deletion policy. | Prevents account deletion from orphaning active classes or erasing student history while keeping deletion available. |
| D-30 | 2026-07-16 | STORE-2 implements one 13+ public student registration gate for Email, Google and Apple; under-13 users require trusted school/guardian provisioning and exact birth dates are not collected. | Prevents provider-specific bypasses and keeps a clear minor-account boundary without turning the release into a Kids Category app. |
